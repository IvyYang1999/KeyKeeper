...
| 备份 | 必须同时保留原登录钥匙串和 KeyKeeper 应用数据，并验证备份钥匙串能够解锁、值能够读取。仅恢复应用文件夹不会恢复密钥。显式的加密导出命令尚未实现；见[恢复说明](docs/STORAGE-RECOVERY.md)。 |

### 一次批准到底允许了什么

你点「允许」之后的确切范围。下面不是推断，是代码真正检查的东西。

| 选项 | 允许了谁 | 允许了什么 | 到什么时候 |
|---|---|---|---|
| 仅本次 | 只有发起请求的那个程序 | 这一条凭据里的全部机密字段 | 下一次成功读取之后 |
| 本次终端会话 | 只有那个程序，且在那个终端会话里 | 同上 | 终端会话结束，或 24 小时，以先到者为准 |
| 1 小时 | 只有发起请求的那个程序 | 同上 | 一小时后 |
| 始终允许 | 只有发起请求的那个程序 | 同上 | 直到你撤销 |

有两点需要说准：

- **「那个程序」是按它从哪儿来认的**，不是按它自报的名字：取进程链上第一个有 bundle 标识的祖先（所以你从终端启动的 Agent，认出来是终端本身），否则取脚本或可执行文件的路径。跑在某个 App 里面的 Agent，被那个 App 的批准覆盖。
- **一次批准覆盖整条凭据，不是单个密钥**。一条凭据里有三个机密字段，为其中一个批准之后，那个程序三个都能读。想分开授权就拆成不同凭据。

网站登录态用同样的三档时长、同样的「只认这个调用方」规则，存放方式也和登录态快照一样——在钥匙串里，不在文件里。它的窗口还会按时结束权限：到点冻结，重新授权之前连不上网络。

KeyKeeper 0.3.4 之前发出的批准没有记录调用方。它们继续有效，而**第一个用到它的程序会成为它的主人**，此后别的程序再也蹭不到。所有批准都能在应用里看到和撤销。
