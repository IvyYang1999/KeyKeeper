...
final class ProviderMarksTests: XCTestCase {
    ...
    func test每个服务商都有主题色_官方标能渲染_其余明确退回字母标() {
        ...
        for id in [
            ...
            "modelscope-cn", "modelscope-global", "novita-ai", "opencode-zen", "opencode-go",
            "pipellm", "relaxycode", "rightcode-codex", "stepfun-api", "stepfun-step-plan",
            "therouter", "xiaomi-mimo-payg", "xiaomi-mimo-token-plan-cn",
            "xiaomi-mimo-token-plan-sg", "xiaomi-mimo-token-plan-eu",
            "baidu-qianfan-cn", "baidu-qianfan-global", "baidu-qianfan-token-plan",
        ] {
            XCTAssertNotNil(ProviderMarks.image(for: id), "verified provider artwork must render for \(id)")
        }
