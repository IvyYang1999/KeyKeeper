...
/// What the app does to its files at every launch, in one place, so a test can run exactly this
/// over files written by an earlier version.
///
/// 【曾经的 bug · 2026-09-13】the two prunes ran straight from AppDelegate, and the first launch
/// after an upgrade wiped every background approval: nobody had ever run the launch sequence
/// over an old data directory before shipping it.
enum LaunchMaintenance {
    struct Stores {
        var meta: MetaStore
        var inventory: () throws -> [String: Set<String>]
        var grants: GrantStore
        var serviceGrants: ServiceGrantStore
    }
    ...
    static func run(_ stores: Stores) {
        ...
        // Expired and spent approvals grant nothing; they only make it harder to see whether
        // anyone has added a line to the file.
        try? stores.grants.pruneExpired()
        try? stores.serviceGrants.pruneExpired()
    }
}
