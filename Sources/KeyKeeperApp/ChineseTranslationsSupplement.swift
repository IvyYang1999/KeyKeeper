...
extension AppL10n {
    static let chineseSupplement: [String: String] = [
        ...
        "An Anthropic API key. The reviewer sees key names, the caller's stated reason and command, and the declared use — never a value. Its opinion is shown in the approval window; it never approves anything.": "一把 Anthropic API key。审查员只看到 key 的名字、请求方写的理由和命令、声明的用途——看不到任何值。它的意见显示在授权窗里，它自己不会批准任何东西。",
        // Dates
        "Today": "今天",
        "Yesterday": "昨天",
    ]
