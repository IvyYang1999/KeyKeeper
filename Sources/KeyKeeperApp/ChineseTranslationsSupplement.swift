...
extension AppL10n {
    static let chineseSupplement: [String: String] = [
        ...
        "No key found at {0} · {1}. Save one there to turn the reviewer on.": "在 {0} · {1} 没找到 key。先把审查员的 key 存到那里。",
        ...
        "Any service that speaks the Anthropic or OpenAI API. The reviewer sees key names, the caller's stated reason and command, and the declared use — never a value. Its opinion is shown in the approval window; it never approves anything.": "任何讲 Anthropic 或 OpenAI 接口的服务都行。审查员只看到 key 的名字、请求方写的理由和命令、声明的用途——看不到任何值。它的意见显示在授权窗里，它自己不会批准任何东西。",
        "Base URL:": "Base URL：",
        "Model:": "模型：",
        "Auto": "自动识别",
