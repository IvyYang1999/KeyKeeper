...
extension AppL10n {
    static let chineseSupplement: [String: String] = [
        ...
        "Any service that speaks the Anthropic or OpenAI API. Its key is stored in KeyKeeper's own Keychain item, not as a credential. The reviewer sees key names, the caller's stated reason and command, and the declared use — never a value. Its opinion is shown in the approval window; it never approves anything.": "任何讲 Anthropic 或 OpenAI 接口的服务都行。它的 key 存在 KeyKeeper 自己的钥匙串条目里，不是一条凭据。审查员只看到 key 的名字、请求方写的理由和命令、声明的用途——看不到任何值。它的意见显示在授权窗里，它自己不会批准任何东西。",
        "API key:": "API key：",
        "saved · paste to replace": "已保存 · 粘贴可替换",
        "paste the reviewer's key": "粘贴审查员的 key",
        "Save key": "保存 key",
        "Forget": "忘掉",
        "Key saved in KeyKeeper's own Keychain item. It is never a credential and never shown again.": "key 已存进 KeyKeeper 自己的钥匙串条目。它不是凭据，也不会再显示。",
